...
public class Exercise7 {
    ...
    public static void main(String[] args) throws IOException {
       ...
       /*ReactiveSources.intNumbersFlux()
    	...
    	//ReactiveSources.intNumbersFlux().flatMap(i -> ReactiveSources.userFlux().filter(e -> e.getId() == i)).subscribe(System.out::println);

        // Print only distinct numbers from intNumbersFluxWithRepeat
    	ReactiveSources.intNumbersFluxWithRepeat().distinct().subscribe(System.out::println);

        // Print from intNumbersFluxWithRepeat excluding immediately repeating numbers
    	ReactiveSources.intNumbersFluxWithRepeat().distinctUntilChanged().log().subscribe(System.out::println);
